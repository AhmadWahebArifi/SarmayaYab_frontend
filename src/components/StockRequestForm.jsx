...
const StockRequestForm = ({ onSubmit, initialData = {} }) => {
  ...
  const fetchSuggestions = async () => {
    try {
      ...
      const [suggestionsRes, reorderRes] = await Promise.all([
        api.get("/analytics/product-suggestions"),
        api.get("/analytics/auto-reorder-suggestions"),
      ]);
  ...
  const updateItemQuantity = (index, quantity) => {
    setFormData((prev) => ({
      ...
      items: prev.items.map((item, i) =>
        i === index
          ? { ...item, requested_qty: parseInt(quantity) || 0 }
          : item,
      ),
    }));
  };
  ...
  const calculateTotal = () => {
    return formData.items
      .reduce(
        (total, item) => total + item.requested_qty * item.selling_price,
        0,
      )
      .toFixed(2);
  };
    ...
    <div className="max-w-4xl mx-auto p-6 bg-surface-container-lowest rounded-xl">
      <div className="mb-8">
        ...
        <p className="text-on-surface-variant">
          Request products from warehouse inventory
        </p>
      </div>
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-on-surface-variant">
                            Current: {product.current_stock} | Suggested:{" "}
